...
int nAnonymizePayDayAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PayDay
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PayDay
    // Mac: ~/Library/Application Support/PayDay
    // Unix: ~/.PayDay
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PayDay";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PayDay.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "PayDayd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong PayDay will not work properly.");
