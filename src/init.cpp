...
std::string HelpMessage()
{
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 7214)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 7215)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizePayDayamount=<n> " + _("Keep N PayDay anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. PayDay is shutting down."));
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of PayDay") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart PayDay to complete") << "\n";
    ...
    nAnonymizePayDayAmount = GetArg("-anonymizePayDayamount", 0);
    if(nAnonymizePayDayAmount > 999999) nAnonymizePayDayAmount = 999999;
    if(nAnonymizePayDayAmount < 2) nAnonymizePayDayAmount = 2;
    ...
    LogPrintf("Anonymize PayDay Amount %d\n", nAnonymizePayDayAmount);
