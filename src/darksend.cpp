...
void CDarksendPool::ProcessMessageDarksend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //DarkSend Accept Into Pool
        if (pfrom->nVersion < GetPoolPeerProtoVersion()) {
        ...
        if(sessionUsers == 0) {
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountMasternodesAboveProtocol(GetPoolPeerProtoVersion())/5 > mnodeman.nDsqCount){
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat PayDay and make it unusable. To
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizePayDayAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if(!sessionFoundMasternode){
        ...
        int nThreshold = (int)(mnodeman.CountEnabled(GetPoolPeerProtoVersion()) * 0.9);
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue){
                ...
                if(protocolVersion < GetPoolPeerProtoVersion()) continue;
        ...
        while(i < 10)
        {
            CMasternode* pmn = mnodeman.FindRandomNotInVec(vecMasternodesUsed, GetPoolPeerProtoVersion());
            if(pmn == NULL)
            {
            ...
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(GetPoolPeerProtoVersion())/5 > mnodeman.nDsqCount){
                i++;
                continue;
            }
...
void ThreadCheckDarkSendPool()
{
    ...
    while (true)
    {
        ...
        if(darkSendPool.IsBlockchainSynced()) {
            ...
            if(atStart || (c % 5 == 0 && RequestedMasterNodeList < 3)){
