...
Value checkkernel(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
        throw JSONRPCError(-9, "PayDay is not connected!");

	if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "PayDay is downloading blocks...");
...
Value mine(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PayDay is not connected!");

	if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PayDay is downloading blocks...");
