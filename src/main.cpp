...
const string strMessageMagic = "PayDay Signed Message:\n";
...
int64_t GetRewardByHeight(int nHeight)
{
	...
	if (nHeight < YEARLY_BLOCKCOUNT)
        ...
        nReward = STATIC_REWARD * 0.75;
	else if (nHeight < (3 * YEARLY_BLOCKCOUNT))
        nReward = STATIC_REWARD * 0.50;
	else if (nHeight < (4 * YEARLY_BLOCKCOUNT))
        nReward = STATIC_REWARD * 0.25;
	else
		nReward = 1 * COIN;
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
	// Base reward.
	int64_t nSubsidy;
	//for balanced network premine
	int64_t nLastPremineBlock=2*STAKE_MIN_CONFIRMATIONS;
	if (nHeight < (nLastPremineBlock+1) )
        	nSubsidy =  (PREMINE_AMOUNT/nLastPremineBlock)* COIN;
	else
		nSubsidy = 0 * COIN;
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
	...
	BOOST_FOREACH(const CTxIn& txin, vin)
	{
		...
		if (IsConfirmedInNPrevBlocks(txindex, pindexPrev, STAKE_MIN_CONFIRMATIONS - 1, nSpendDepth))
		{
...
bool CBlock::AcceptBlock()
{
	...
	if (hashBestChain == hash)
	{
		...
		BOOST_FOREACH(CNode* pnode, vNodes)
			if (nBestHeight > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 250 : nBlockEstimate))
