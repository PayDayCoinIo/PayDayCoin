...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizePayDayAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePayDayAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizePayDayAmount = strAnonymizePayDayAmount.remove(strAnonymizePayDayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePayDayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizePayDayAmount*COIN) nMaxToAnonymize = nAnonymizePayDayAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizePayDayAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizePayDayAmount));
        strAnonymizePayDayAmount = strAnonymizePayDayAmount.remove(strAnonymizePayDayAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePayDayAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
