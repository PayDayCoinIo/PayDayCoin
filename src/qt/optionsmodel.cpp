...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nAnonymizePayDayAmount"))
        settings.setValue("nAnonymizePayDayAmount", 1000);
    nAnonymizePayDayAmount = settings.value("nAnonymizePayDayAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizePayDayAmount"))
        SoftSetArg("-anonymizePayDayamount", settings.value("nAnonymizePayDayAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePayDayAmount:
            return QVariant(nAnonymizePayDayAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizePayDayAmount:
            nAnonymizePayDayAmount = value.toInt();
            settings.setValue("nAnonymizePayDayAmount", nAnonymizePayDayAmount);
            emit AnonymizePayDayAmountChanged(nAnonymizePayDayAmount);
            break;
        default:
            break;
        }
